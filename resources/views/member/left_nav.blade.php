<div class="mainl">
    <ul>
        <li class="cur tit"><a href="{{ url('member/zqm/center') }}">委托方会员</a></li>
        <li><a href="{{ url('member/center/sysinfo') }}" target="_blank">系统消息</a></li>
        <li><a href="{{ url('member/zqm/index') }}" target="_blank">发布资产信息</a></li>
        <li><a href="{{ url('member/zqList') }}" target="_blank">我发布的信息</a></li>
        <li><a href="{{ url('member/center/apply') }}" target="_blank">已申请信息</a></li>
        <li><a href="{{ url('member/center/collect') }}" target="_blank">已收藏信息</a></li>

        
        <li><a href="{{url('member/authenticate/index')}}"  target="_blank">资格认证</a></li>
        <li><a href="{{ url('member/center/security') }}" target="_blank">安全中心</a></li>
        <li><a href="{{ url('/auth/logout') }}">安全退出</a></li>
    </ul>
</div>